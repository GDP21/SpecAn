...
)
{
    ...
    if(err == TM_OK)
    {
        ...
        if(pObj->StandardMode>TDA18273_StandardMode_Unknown && pObj->StandardMode<TDA18273_StandardMode_Max)
        {
            ...
            {
                /* Set AGC3 RF AGC Top */
                err = iTDA18273_Write(pObj, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Top, pObj->pStandard->AGC3_TOP_I2C, Bus_None);
                tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "iTDA18273_Write(0x%08X) failed.", pObj->tUnitW));
            }
            ...
            {
                /* Set AGC3 Adapt TOP */
                err = iTDA18273_Write(pObj, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Adapt_TOP, pObj->pStandard->AGC3_Adapt_TOP, Bus_None);
                tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "iTDA18273_Write(0x%08X) failed.", pObj->tUnitW));
            }
