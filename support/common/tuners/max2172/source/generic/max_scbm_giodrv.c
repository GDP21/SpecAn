...
/* Sense of the GPIO to control the standby signal */
#define STANDBY GPIO_LEVEL_LOW
#define ENABLED GPIO_LEVEL_HIGH

#ifndef STBY_GPIO_PIN
#define	STBY_GPIO_PIN	GPIO_PIN_PDM_B_PIN
#define	STBY_GPIO_BLOCK	GPIO_PIN_PDM_B_BLOCK
#endif

/* Handle to indentify the GPIO line that is used as STBY signal */
static GPIO_PIN_T STBYhandle;
...
int MAX_shutdownPort(void)
{
	...
	if (GPIO_STATUS_OK != GPIORemovePin(&STBYhandle))
	{
		return 0;
	}
...
** FUNCTION:    MAX_enable
**
** DESCRIPTION: Enables the RF, ie takes it out of standby
**
** RETURNS:     int : 1 for success, 0 for failure
**
*/
int MAX_enable(void)
{
	if (GPIOSet(&STBYhandle, ENABLED) != GPIO_STATUS_OK)
		return 0;
	else
		return 1;
}
...
/*
** FUNCTION:    MAX_standby
**
** DESCRIPTION: Puts the RF into a standby state
**
** RETURNS:     int : 1 for success, 0 for failure
**
*/
...
{
	if (GPIOSet(&STBYhandle, STANDBY) != GPIO_STATUS_OK)
		return 0;
	else
		return 1;
}
